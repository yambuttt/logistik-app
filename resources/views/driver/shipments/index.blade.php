            ...
            <div class="glass-panel rounded-[24px] border border-white/50 p-6 shadow-lg">
                <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                    <div class="space-y-2">
                        ...
                        @if ($shipment->vehicle)
                            <p class="text-sm text-slate-500">
                                Kendaraan: <span class="font-semibold text-slate-800">{{ $shipment->vehicle->name }} - {{ $shipment->vehicle->plate_number }}</span>
                            </p>
                        @endif
                    ...
                    <div class="space-y-3">
                        <div>
                            @if ($shipment->status === 'assigned')
                                <span class="rounded-full bg-sky-100 px-3 py-1 text-xs font-semibold text-sky-700">Assigned</span>
                            @elseif ($shipment->status === 'on_delivery')
                                <span class="rounded-full bg-amber-100 px-3 py-1 text-xs font-semibold text-amber-700">On Delivery</span>
                            ...
                            @elseif ($shipment->status === 'returning')
                                <span class="rounded-full bg-indigo-100 px-3 py-1 text-xs font-semibold text-indigo-700">Returning</span>
                            @else
