...
<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <h2 class="text-2xl font-bold text-slate-900">Waste</h2>
            <p class="mt-1 text-sm text-slate-500">Catatan waste untuk gudang Anda.</p>
        </div>

        <a href="{{ route('warehouse.wastes.create') }}"
                ...
                <tbody>
                    @forelse ($wastes as $waste)
                        <tr class="rounded-2xl bg-white shadow-sm">
                            <td class="rounded-l-2xl px-4 py-4 font-semibold text-slate-900">{{ $waste->waste_number }}</td>
                            <td class="px-4 py-4 text-sm text-slate-600">{{ \Carbon\Carbon::parse($waste->waste_date)->format('d M Y') }}</td>
                            <td class="px-4 py-4 text-sm font-semibold text-slate-800">{{ $waste->product->name ?? '-' }}</td>
                            <td class="px-4 py-4 text-sm">
                                <span class="rounded-full bg-red-100 px-3 py-1 text-xs font-semibold text-red-700">
                                    {{ rtrim(rtrim(number_format($waste->qty, 2, '.', ''), '0'), '.') }}
                                </span>
                            </td>
                            <td class="rounded-r-2xl px-4 py-4 text-sm text-slate-600">{{ $waste->reason }}</td>
                        </tr>
                    @empty
                        <tr><td colspan="5" class="px-4 py-10 text-center text-sm text-slate-500">Belum ada transaksi waste.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>

        <div class="mt-6">{{ $wastes->links() }}</div>
    </div>
</div>
@endsection